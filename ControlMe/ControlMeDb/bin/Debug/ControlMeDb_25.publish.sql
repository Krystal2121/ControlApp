/*
Deployment script for ControlMe

This code was generated by a tool.
Changes to this file may cause incorrect behavior and will be lost if
the code is regenerated.
*/

GO
SET ANSI_NULLS, ANSI_PADDING, ANSI_WARNINGS, ARITHABORT, CONCAT_NULL_YIELDS_NULL, QUOTED_IDENTIFIER ON;

SET NUMERIC_ROUNDABORT OFF;


GO
:setvar DatabaseName "ControlMe"
:setvar DefaultFilePrefix "ControlMe"
:setvar DefaultDataPath "D:\Program Files\Microsoft SQL Server\MSSQL14.MSSQLSERVER\MSSQL\DATA\"
:setvar DefaultLogPath "D:\Program Files\Microsoft SQL Server\MSSQL14.MSSQLSERVER\MSSQL\DATA\"

GO
:on error exit
GO
/*
Detect SQLCMD mode and disable script execution if SQLCMD mode is not supported.
To re-enable the script after enabling SQLCMD mode, execute the following:
SET NOEXEC OFF; 
*/
:setvar __IsSqlCmdEnabled "True"
GO
IF N'$(__IsSqlCmdEnabled)' NOT LIKE N'True'
    BEGIN
        PRINT N'SQLCMD mode must be enabled to successfully execute this script.';
        SET NOEXEC ON;
    END


GO
USE [$(DatabaseName)];


GO
PRINT N'Rename refactoring operation with key 5be56025-728c-4c44-9eec-73c5a5830ec7 is skipped, element [dbo].[UsersGroups].[GroupID] (SqlSimpleColumn) will not be renamed to GroupRefId';


GO
PRINT N'Altering Table [dbo].[CmdLog]...';


GO
ALTER TABLE [dbo].[CmdLog]
    ADD [GroupRefId] INT NULL;


GO
PRINT N'Altering Table [dbo].[ControlAppCmd]...';


GO
ALTER TABLE [dbo].[ControlAppCmd]
    ADD [GroupRefId] INT NULL;


GO
PRINT N'Altering Procedure [dbo].[USP_SendAppCmd]...';


GO

ALTER PROCEDURE [dbo].[USP_SendAppCmd]
@usernm varchar(250),
@usercmd varchar(max),
@all int =0,
@username varchar(250) = null,
@password varchar(250) = null,
@fileloc varchar(max) = null
AS
BEGIN


Declare @senderid int =0
Declare @cmdid int =0
if (@username is not null)
BEGIN
	if (@username!='')
	BEGIN
		select @senderid=
		(
			select top 1 Id from
			(
				SELECT Id, [Role] from Users where [Login Name]=@UserName and Password=@Password
				union SELECT Id, [Role] from Users where [Screen Name]=@UserName and Password=@Password
			) as subq
		)
	END
	ELSE
	BEGIN
	SET @senderid=-1
	END
END
IF (@senderid!=0)
BEGIN
	declare @id int
	IF (@usernm='')
	BEGIN
		if (@all=0)
		BEGIN
			declare @numbers int
			SELECT @numbers=count(*) from Users where  [RandOpt]=1

			declare @which int
			select @which =cast(floor(rand()*@numbers) as int)+1

			SELECT @id=Id FROM Users
			ORDER BY Id
			OFFSET @which-1 ROWS
			FETCH NEXT 1 ROWS ONLY;

			insert into ControlAppCmd
			([SenderId],
			[SubId],
			[Content])
			values (@senderid,@id,@usercmd)

			insert into CmdLog
			([SenderId],[SubId],
			[Content])
			values (@senderid,@id,@usercmd)
		END
		ELSE
		BEGIN
			insert into ControlAppCmd
			([SenderId],
			[SubId],
			[Content])
			select @senderid,Id, @usercmd
			from Users
			where  [RandOpt]=1

			insert into CmdLog
			([SenderId],[SubId],
			[Content])
			values (@senderid,-1,@usercmd)
		END

	END
	ELSE
	IF ((ISNUMERIC(@usernm)=1) and (CONVERT(int,@usernm)<-1))
	BEGIN
		insert into ControlAppCmd
		([SenderId],
		[SubId],
		[Content],
		[GroupRefId])
		select @senderid,UserId, @usercmd,CONVERT(int,@usernm)
		from UsersGroups
		where GroupRefId=CONVERT(int,@usernm)

		insert into CmdLog
		([SenderId],[SubId],
		[Content],
		[GroupRefId])
		values (@senderid,@senderid,@usercmd,CONVERT(int,@usernm))
	END
	ELSE
	BEGIN

		select @id=(
		select top 1 Id from
		( Select  Id from Users where [Screen Name]=@usernm union Select  Id from Users where convert(varchar(250),Id)=@usernm) sub1
		)
		if(@id is not null)
		BEGIN
			insert into ControlAppCmd
			([SenderId],[SubId],
			[Content])
			values (@senderid,@id,@usercmd)

			insert into CmdLog
			([SenderId],[SubId],
			[Content])
			values (@senderid,@id,@usercmd)
		END
	END
END

delete from [ControlAppCmd]
where [SenderId]<0
and [Content] like '%1mp7JCqszmk=|||%'

delete from [ControlAppCmd] where content =''

END
GO
PRINT N'Altering Procedure [dbo].[USP_GetOutstanding]...';


GO
ALTER PROCEDURE [dbo].[USP_GetOutstanding]
	@userID int = 0
AS
BEGIN

		DELETE cmd FROM ControlAppCmd cmd
		WHERE SenderId in (Select BlockeeId FROM Block where BlockerId=@userID)
		AND SubId=@userID

		DECLARE @Anon bit = 0
		SELECT @Anon=AnonCmd from Users where ID=@userID

		IF (@Anon=0)
		BEGIN
			DELETE cmd FROM ControlAppCmd cmd
			WHERE SenderId =-1
			AND SubId=@userID
		END
		declare @whonext varchar(20)
		select top 1 @whonext= isnull(GroupRefId,SenderId) from ControlAppCmd where SubId=@userID order by [SendDate]
		if Convert(int,@whonext )<-1
		begin
		select @whonext= 'Group: '& GroupName from Groups where RefId=@whonext
		end
		else
		if Convert(int,@whonext )=-1
		begin
		set @whonext='Anon'
		end
		else
		begin
		set @whonext='User'
		end
		SELECT convert(varchar(100),count(*)) counting,  @whonext whonext  from ControlAppCmd where SubId=@userID
END
GO
PRINT N'Creating Procedure [dbo].[USP_DeleteOutstanding]...';


GO
CREATE PROCEDURE [dbo].[USP_DeleteOutstanding]
	@userID int = 0
AS
BEGIN

		DELETE cmd FROM ControlAppCmd cmd
		WHERE SubId=@userID

END
GO
PRINT N'Refreshing Procedure [dbo].[USP_CmdComplete]...';


GO
EXECUTE sp_refreshsqlmodule N'[dbo].[USP_CmdComplete]';


GO
PRINT N'Refreshing Procedure [dbo].[USP_GetAppContent]...';


GO
EXECUTE sp_refreshsqlmodule N'[dbo].[USP_GetAppContent]';


GO
-- Refactoring step to update target server with deployed transaction logs
IF NOT EXISTS (SELECT OperationKey FROM [dbo].[__RefactorLog] WHERE OperationKey = '5be56025-728c-4c44-9eec-73c5a5830ec7')
INSERT INTO [dbo].[__RefactorLog] (OperationKey) values ('5be56025-728c-4c44-9eec-73c5a5830ec7')

GO

GO
PRINT N'Update complete.';


GO
