...
GO
PRINT N'Creating Table [dbo].[UsersGroups]...';
...
BEGIN
		...
		select top 1 @whonext= case when SenderId>0 then 'User' when SenderId<-1 then 'Group' when SenderId=-1 then 'Anon' else 'N/A' end from ControlAppCmd where SubId=@userID order by [SendDate]
		SELECT convert(varchar(100),count(*)) counting, @whonext from ControlAppCmd where SubId=@userID
